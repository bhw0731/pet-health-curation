    ...
    <div className="bg-stone-50 py-10 sm:py-14">
      <div className="mx-auto max-w-5xl px-4">
        <header className="mb-6">
          <h1 className="text-2xl font-extrabold text-stone-900 sm:text-3xl">콘텐츠</h1>
          <p className="mt-1.5 text-stone-500">수의사 감수를 거친 믿을 수 있는 반려 정보 📚</p>
        </header>
        ...
        {feature && (
          <article className="card mt-5 overflow-hidden sm:flex">
            ...
            <div className="p-6 sm:w-3/5">
              ...
              <h2 className="mt-3 text-xl font-bold text-stone-900">{feature.title}</h2>
              <p className="mt-2 text-sm leading-relaxed text-stone-500">{feature.excerpt}</p>
              <p className="mt-4 text-xs text-stone-400">읽는 시간 {feature.read}</p>
            </div>
          </article>
        )}
          ...
          {rest.map((a) => (
            <article key={a.id} className="card overflow-hidden transition hover:shadow-md">
              ...
              <div className="p-4">
                ...
                <h3 className="mt-1 font-bold text-stone-900">{a.title}</h3>
                <p className="mt-1 line-clamp-2 text-sm text-stone-500">{a.excerpt}</p>
                <p className="mt-3 text-xs text-stone-400">읽는 시간 {a.read}</p>
              </div>
            </article>
          ))}
        </div>
