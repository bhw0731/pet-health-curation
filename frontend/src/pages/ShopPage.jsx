    ...
    <div className="bg-slate-50 py-10 sm:py-14">
      <div className="mx-auto max-w-5xl px-4">
        <header className="mb-6 flex items-end justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 sm:text-3xl">쇼핑</h1>
            <p className="mt-1.5 text-slate-500">우리 아이 고민에 맞는 제품을 만나보세요 🛍️</p>
          </div>
          <span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-400">AD · 제휴</span>
        </header>
          ...
          {items.map((p) => (
            <article key={p.id} className="card overflow-hidden transition hover:shadow-md">
              ...
              <div className="p-3">
                ...
                <p className="mt-0.5 line-clamp-2 text-sm font-semibold text-slate-800">{p.name}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-sm font-extrabold text-slate-900">{won(p.price)}</span>
        ...
        {toast && (
          <div className="fixed bottom-6 left-1/2 z-40 -translate-x-1/2 rounded-full bg-slate-900 px-5 py-3 text-sm font-medium text-white shadow-lg">
            {toast}
          </div>
        )}
        <p className="mt-6 text-center text-[11px] text-slate-300">제휴 링크를 통해 구매 시 일정 수수료가 발생할 수 있어요.</p>
      </div>
    </div>
  );
}
