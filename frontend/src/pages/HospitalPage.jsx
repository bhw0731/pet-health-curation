    ...
    <div className="bg-slate-50 py-10 sm:py-14">
      <div className="mx-auto max-w-3xl px-4">
        <header className="mb-6">
          <h1 className="text-2xl font-extrabold text-slate-900 sm:text-3xl">동물병원 찾기</h1>
          <p className="mt-1.5 text-slate-500">우리 동네 가까운 병원을 찾아보세요 🏥</p>
        </header>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 py-3 pl-11 pr-4 outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100"
          />
        </div>
          ...
          {REGIONS.map((r) => (
            <button
              ...
              className={`flex-shrink-0 rounded-full px-4 py-1.5 text-sm font-semibold transition ${
                region === r ? 'bg-slate-900 text-white' : 'bg-white text-slate-500 ring-1 ring-slate-100'
              }`}
            >
              {r}
            </button>
        ...
        {/* 목록 */}
        <p className="mt-5 text-sm text-slate-400">총 {list.length}곳</p>
            ...
            <article key={h.id} className="card p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex-shrink-0 text-right">
                  ...
                  <p className="text-xs text-slate-400">후기 {h.reviews}</p>
                </div>
              </div>
              <p className="mt-2 text-sm text-slate-500">🕒 {h.hours}</p>
        ...
        <p className="mt-6 text-center text-xs text-slate-400">표시된 병원은 데모용 예시 데이터입니다.</p>
      </div>
    </div>
  );
}
