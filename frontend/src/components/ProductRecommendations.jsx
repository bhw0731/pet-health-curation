  ...
  return (
    <section className="mt-8 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-lg font-bold text-slate-900">🛒 {petName}에게 딱 맞는 추천 제품</h4>
          <p className="text-xs text-slate-400">분석된 고민에 맞춰 큐레이션했어요</p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        {products.map((p, i) => (
          <article
            key={i}
            className="flex items-center gap-3 rounded-xl border border-slate-100 p-3 transition hover:border-brand-200 hover:shadow-sm"
          >
            {/* 상품 썸네일 (플레이스홀더) */}
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="truncate text-[11px] text-slate-400">{p.brand}</span>
              </div>
              <p className="mt-0.5 truncate text-sm font-semibold text-slate-800">{p.name}</p>
              <div className="mt-1 flex items-center justify-between">
                <span className="text-base font-extrabold text-slate-900">{won(p.price)}</span>
                <button
