...
export default function ProfileCard({ profile, onSave, onClear }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      <p className="text-sm font-bold text-slate-800">🐾 나의 반려동물 프로필 만들기</p>
        ...
        {AVATARS.map((a) => (
          <button
            ...
            className={`grid h-11 w-11 place-items-center rounded-xl border-2 text-xl transition ${
              avatar === a ? 'border-brand-500 bg-brand-50' : 'border-slate-200 hover:border-slate-300'
            }`}
          >
            {a}
          </button>
      ...
      <div className="mt-4 flex gap-2">
        <input
          ...
          placeholder="이름 (예: 콩이)"
          className="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100"
        />
        <button
          onClick={handleSave}
          disabled={!name.trim()}
